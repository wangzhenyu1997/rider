...
//        把原图按照比例放大缩小到ImageView的高度，显示在ImageVIew的end（后部/尾部/底部）
    ...
    //默认缓存图片
    public static RequestOptions getHeadImgOption() {
        RequestOptions options = new RequestOptions()
                .placeholder(R.mipmap.icon_personal)
                .error(R.mipmap.icon_personal)
                .circleCrop()//加载成圆形
                .skipMemoryCache(true)//禁用内存缓存
                .diskCacheStrategy(DiskCacheStrategy.ALL);
        return options;
    ...
    //默认缓存图片
    public static RequestOptions getRoundedOption(int round) {
        RequestOptions options = new RequestOptions()
                .placeholder(R.mipmap.default_image)
                .error(R.mipmap.default_image)

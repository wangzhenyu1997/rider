...
public class TagAliasOperatorHelper {
            ...
            switch (msg.what){
                case DELAY_SEND_ACTION:
                    if(msg.obj !=null && msg.obj instanceof  TagAliasBean){
                        ...
                        setActionCache.put(sequence, tagAliasBean);
                        if(context!=null) {
                            handleAction(context, sequence, tagAliasBean,mCallBack);
                        }else{
     ...
     * 处理设置tag
     * */
    public void handleAction(Context context, int sequence, TagAliasBean tagAliasBean, completeAll callBack){
    ...
    public void onTagOperatorResult(Context context, JPushMessage jPushMessage) {
        ...
        if(jPushMessage.getErrorCode() == 0){
            ...
            Logger.i(TAG,logs);
//            ExampleUtil.showToast(logs, context);//成功展示结果
            mCallBack.complete(logs);
        }else{
        	mCallBack.complete("-1");//代表失败

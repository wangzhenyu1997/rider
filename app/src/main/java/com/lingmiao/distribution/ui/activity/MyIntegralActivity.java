...
import com.lingmiao.distribution.databinding.ActivityMyIntegralBinding;
 ...
 * niuxinyu
 */
public class MyIntegralActivity extends ActivitySupport {

    private ActivityMyIntegralBinding viewBinding;
    ...
    protected void setCreateView(@Nullable Bundle savedInstanceState) {
        viewBinding = ActivityMyIntegralBinding.inflate(LayoutInflater.from(this));
    ...
    private void getData() {
        ...
        OkHttpUtils.postAsyn(Constant.AppIntegralIndex, mMap, new HttpCallback<WalletBean>() {
            @Override
            public void onSuccess(WalletBean response) {
                super.onSuccess(response);
                if (response.getCode().equals(Constant.SUCCESS)) {
                    viewBinding.amiIntegral.setText(MathExtend.round(response.getData().getIntegralAccount().getBalanceAmount(), 2));
                    dataId = response.getData().getIntegralAccount().getId();
                    getDataList();
                } else {

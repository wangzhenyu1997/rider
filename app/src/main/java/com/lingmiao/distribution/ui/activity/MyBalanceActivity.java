...
import com.lingmiao.distribution.databinding.ActivityMyBalanceBinding;
 ...
 * niuxinyu
 */
public class MyBalanceActivity extends ActivitySupport implements View.OnClickListener {

    private ActivityMyBalanceBinding viewBinding;
    ...
    protected void setCreateView(@Nullable Bundle savedInstanceState) {
        viewBinding = ActivityMyBalanceBinding.inflate(LayoutInflater.from(this));
    ...
    private void initView() {
        ...
        viewBinding.ambRecharge.setOnClickListener(this);
        viewBinding.ambWithdrawal.setOnClickListener(this);
    }
    ...
    private void getData() {
        ...
        OkHttpUtils.postAync(Constant.AppBalanceIndex, new Gson().toJson(mMap), new HttpCallback<WalletBean>() {
            @Override
            public void onSuccess(WalletBean response) {
                super.onSuccess(response);
                if (response.getCode().equals(Constant.SUCCESS)) {
                    viewBinding.ambBalance.setText(MathExtend.round(response.getData().getBalanceAccount().getBalanceAmount(), 2));
                    dataId = response.getData().getBalanceAccount().getId();
                    getDataList();
                } else {
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            case R.id.amb_recharge:
                startActivity(new Intent(context, RechargeActivity.class).putExtra("dataId", dataId));
                break;
            case R.id.amb_withdrawal:
                startActivity(new Intent(context, WithdrawalActivity.class).putExtra("accountId", dataId));
                break;
        }
    }

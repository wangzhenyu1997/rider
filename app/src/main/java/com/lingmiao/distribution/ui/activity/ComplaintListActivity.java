...
import com.lingmiao.distribution.databinding.ActivityComplaintListBinding;
 ...
 * niuxinyu
 */
public class ComplaintListActivity extends ActivitySupport {

    private ActivityComplaintListBinding viewBinding;
    ...
    protected void setCreateView(@Nullable Bundle savedInstanceState) {
        viewBinding = ActivityComplaintListBinding.inflate(LayoutInflater.from(this));
    ...
    private void initView() {
        ...
        mAdapter = new CommonAdapter<ComplaintParam>(this, R.layout.item_complaint, mData) {
            @Override
            protected void convert(ViewHolder holder, final ComplaintParam data, int position) {
                holder.setText(R.id.m_tittle, data.getTypeName());
                if (data.getComplaintObj() == 1) {  //投诉我的
                    holder.setText(R.id.m_tittle_tag, "投诉我的");
                    holder.setTextColorRes(R.id.m_tittle_tag, R.color.colorPrice);
                    holder.setBackground(R.id.m_tittle_tag, R.drawable.xml_border_tag_im_red_frame);
                } else {
                    holder.setText(R.id.m_tittle_tag, "我投诉的");
                    holder.setTextColorRes(R.id.m_tittle_tag, R.color.im_color);
                    holder.setBackground(R.id.m_tittle_tag, R.drawable.xml_border_tag_im_frame);
                }
                holder.setText(R.id.m_time, data.getCreateTime());
                holder.setText(R.id.m_content, data.getExplainReason());
            }
        };
    ...
    private void getData() {
        ...
        OkHttpUtils.postAync(Constant.AppComplaintList, new Gson().toJson(mMap), new HttpCallback<ComplaintBean>(context, getProgressDialog()) {
            @Override

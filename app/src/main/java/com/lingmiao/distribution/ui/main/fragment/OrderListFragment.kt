...
import com.lingmiao.distribution.ui.main.presenter.impl.OrderVieListPreImpl
...
Desc        :
 **/
class OrderListFragment : BaseLoadMoreFragment<DispatchOrderItemBean, IOrderViePresenter>(),
    IOrderViePresenter.View {
                ...
                when (view.id) {
                    ...
                    R.id.tv_dispatch_start_distance -> {
                        ...
                        MapNav.chooseMapDialog(
                            requireContext(),
                            item?.getConsignerAddressStr(),
                            item?.consignerLat ?: 0.0,
                            item?.consignerLng ?: 0.0
                        )
                    }
                    R.id.tv_dispatch_end_distance -> {
                        ...
                        MapNav.chooseMapDialog(
                            requireContext(),
                            item?.getConsigneeAddressStr(),
                            item?.consigneeLat ?: 0.0,
                            item?.consigneeLng ?: 0.0
                        )
                    }
                }
            }
    ...
    override fun upDateTotalCount(num: String) {
        number = num
    }
    ...
    override fun onLoadMoreSuccess(list: List<DispatchOrderItemBean>?, hasMore: Boolean) {
        ...
        EventBus.getDefault().post(DispatchSingleNumberEvent(-1, temp))
    }

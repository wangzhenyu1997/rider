...
import com.lingmiao.distribution.databinding.ActivityRejectionBinding;
 ...
 * niuxinyu
 */
public class RejectionActivity extends ActivitySupport implements View.OnClickListener {

    private ActivityRejectionBinding viewBinding;
    ...
    protected void setCreateView(@Nullable Bundle savedInstanceState) {
        viewBinding = ActivityRejectionBinding.inflate(LayoutInflater.from(this));
    ...
    private void getReason() {
        ...
        mMap.put("type", "refuse_accept_type");
    ...
    private void submit() {
        ...
        if (mRefuseAcceptType.isEmpty()) {
            ToastUtil.showToast(context, "请选择拒单原因！");
            return;
        }
        ...
        mMap.put("refuseAcceptType", mRefuseAcceptType);
        mMap.put("explainReason", viewBinding.mContent.getText().toString());
        OkHttpUtils.postAync(Constant.AppRefuseAccept, new Gson().toJson(mMap), new HttpCallback<BasicBean>(context, getProgressDialog()) {
            @Override
